...
//! `gmp::mp_set_memory_functions` becomes
//! `gmp::set_memory_functions`, but `gmp::mpz_init` does *not* become
//! `gmp::z_init`. Also, the types `mpfr::mpfr_t` and `mpc::mpc_t` are
//! *not* shortened to `mpfr::t` or `mpc::t`.
//! 
//! ## Types
//! 
