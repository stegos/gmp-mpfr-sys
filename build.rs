...
fn build_gmp(env: &Environment, lib: &Path, header: &Path) {
    ...
    let mut configure = Command::new("../gmp-src/configure");
    configure
...
fn build_mpfr(env: &Environment, lib: &Path, header: &Path) {
    ...
    let mut configure = Command::new("../mpfr-src/configure");
    configure
        .current_dir(&build_dir)
        .env("CC", &env.cc)
        .env("CFLAGS", &env.cflags)
...
fn build_mpc(env: &Environment, lib: &Path, header: &Path) {
    ...
    println!("$ export CC={:?}", &env.cc);
    println!("$ export CFLAGS={:?}", &env.cflags);
    let mut configure = Command::new("../mpc-src/configure");
    configure
        .current_dir(&build_dir)
        .env("CC", &env.cc)
        .env("CFLAGS", &env.cflags)
        ...
        .arg("--disable-shared")
        .arg("--with-pic")
        .arg("--with-mpfr-include=../mpfr-src/src")
        .arg("--with-mpfr-lib=../mpfr-build/src/.libs")
        .arg("--with-gmp-include=../gmp-build")
        .arg("--with-gmp-lib=../gmp-build/.libs");
    execute(configure);
    make_and_check(env, &build_dir);
